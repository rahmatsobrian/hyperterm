...
impl AnsiParser {
    ...
    /// into `vbuf`. Returns any bytes the *terminal itself* must write back
    /// to the session (e.g. a Device Status Report cursor-position reply)
    /// -- these are protocol handshake bytes the remote/shell side is
    /// blocking on, not something the caller has any choice about sending.
    pub fn feed(&mut self, bytes: &[u8], core: &mut TerminalCore, vbuf: &mut VirtualBuffer) -> Vec<u8> {
        let mut performer = Performer { core, vbuf, replies: Vec::new() };
        for byte in bytes {
...
struct Performer<'a> {
    ...
    /// Bytes queued up to be written back to the session by `feed`'s
    /// caller (DSR replies, etc.) -- see module docs.
    replies: Vec<u8>,
}
    ...
    fn csi_dispatch(
        ...
        match action {
            ...
            'n' => match p.first().copied().unwrap_or(0) {
                // DSR "report cursor position" -- ConPTY (and plenty of
                // real interactive programs: vim, htop, etc.) send this
                // right at startup and then block waiting for our reply
                // before producing any further output. Without this the
                // session looks completely dead: exactly this sequence is
                // the only thing that ever arrives. Reply is 1-based
                // row/col, terminated the same way a real terminal does.
                6 => {
                    let (row, col) = self.core.cursor();
                    self.replies
                        .extend(format!("\x1b[{};{}R", row + 1, col + 1).into_bytes());
                }
                // DSR "report device OK" -- always reply OK.
                5 => {
                ...
                _ => {
                    tracing::trace!(
                        ...
                        "unhandled DSR request: params={:?}", p
                    );
                }
            },
            _ => {
