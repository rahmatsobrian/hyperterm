...
//!
//! ## Scope (v1)
//! One SSH session per window (no tabs/splits yet -- the console
//! renderer's tab/split machinery in `session_manager` isn't reused here;
//! see ROADMAP.md). A login screen collects host/port/user/auth method,
//! then swaps to a scrollable, resizable terminal view once connected.
//! Bold text and font selection are not implemented yet -- only color,
//! underline, strikethrough, and reverse-video are rendered.
//!
//! ## Architecture
...
//! server to test the actual window against. It's built directly on top
//! of the already-exercised `terminal_core`/`ansi_parser`/`virtual_buffer`
//! pipeline (covered by the existing test suite), but the eframe/egui
//! integration itself -- layout, key mapping, glyph metrics -- has only
//! been reviewed, not run. Expect to iterate against real CI/manual
//! testing feedback.
...
/// Messages sent from the UI thread to the dedicated SSH thread.
enum GuiToSsh {
    Input(Vec<u8>),
    Resize(u32, u32),
    Close,
...
/// Messages sent from the dedicated SSH thread back to the UI thread.
enum SshToGui {
    Connected,
...
/// Spawns the dedicated SSH thread (see module docs) and returns the two
...
fn spawn_ssh_thread(
    ...
    mpsc::UnboundedSender<GuiToSsh>,
    mpsc::UnboundedReceiver<SshToGui>,
) {
    let (to_ssh_tx, mut to_ssh_rx) = mpsc::unbounded_channel::<GuiToSsh>();
    let (from_ssh_tx, from_ssh_rx) = mpsc::unbounded_channel::<SshToGui>();

    std::thread::spawn(move || {
        let rt = match tokio::runtime::Runtime::new() {
            Ok(rt) => rt,
            Err(e) => {
                let _ = from_ssh_tx.send(SshToGui::ConnectFailed(format!(
        ...
        rt.block_on(async move {
            let mut ssh = match SshSession::connect(params, cols, rows).await {
                Ok(s) => s,
                Err(e) => {
                    let _ = from_ssh_tx.send(SshToGui::ConnectFailed(format!("{e:#}")));
                    return;
                }
            };
            if from_ssh_tx.send(SshToGui::Connected).is_err() {
            ...
            loop {
                tokio::select! {
                    ...
                    ev = ssh.next_event() => {
                        match ev {
                            Some(SshEvent::Data(d)) | Some(SshEvent::ExtendedData(d)) => {
                                if from_ssh_tx.send(SshToGui::Data(d)).is_err() {
                                    return;
                                }
                            }
                            Some(SshEvent::ExitStatus(_)) => {}
                            Some(SshEvent::Eof) | Some(SshEvent::Closed) | None => {
                                let _ = from_ssh_tx.send(SshToGui::Closed);
                                return;
                            }
                            Some(SshEvent::Disconnected { reason }) => {
                                let _ = from_ssh_tx.send(SshToGui::Disconnected(reason));
                                return;
                            }
                        }
                    }
                    cmd = to_ssh_rx.recv() => {
                        match cmd {
                            Some(GuiToSsh::Input(bytes)) => {
                                let _ = ssh.send_input(&bytes).await;
                            }
                            Some(GuiToSsh::Resize(c, r)) => {
                                let _ = ssh.resize(c, r).await;
                            }
                            Some(GuiToSsh::Close) | None => {
                                let _ = ssh.close().await;
                                return;
                            }
                        }
...
struct Terminal {
    ...
    parser: AnsiParser,
    vbuf: VirtualBuffer,
    to_ssh: mpsc::UnboundedSender<GuiToSsh>,
    from_ssh: mpsc::UnboundedReceiver<SshToGui>,
    title: String,
...
impl App {
    ...
    fn try_connect(&mut self) {
        ...
        let auth = match launcher.auth_mode {
            ...
            AuthMode::Key => {
                if launcher.identity_path.trim().is_empty() {
                    launcher.error = Some("identity file path can't be empty".to_string());
                    return;
                }
        ...
        let params = ConnectParams {
            ...
            // `Prompt` (the CLI default) does a blocking `stdin` read to
            // ask about new/changed host keys -- there's no console
            // attached to prompt into here, and no host-key-confirmation
            // dialog in the GUI yet (see ROADMAP.md), so `Prompt` would
            // just hang forever on the first connection to any new host.
            // `Tofu` (trust-on-first-use) is the only policy that works
            // without one; a *changed* key is still refused either way.
        ...
        let (to_ssh, from_ssh) = spawn_ssh_thread(params, cols as u32, rows as u32);
...
impl App {
    fn update_launcher(&mut self, ctx: &egui::Context) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.vertical_centered(|ui| {
                ...
                ui.label("SSH terminal -- new session");
            });
            ui.add_space(16.0);
            ...
            egui::Grid::new("launcher_grid")
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ui.radio_value(&mut launcher.auth_mode, AuthMode::Password, "Password");
    ...
    fn update_terminal(&mut self, ctx: &egui::Context) {
        ...
        // Drain everything the SSH thread has sent since the last frame.
        let mut back_to_launcher = false;
        while let Ok(msg) = term.from_ssh.try_recv() {
            match msg {
                SshToGui::Connected => term.status = None,
                SshToGui::Data(bytes) => {
                    term.parser.feed(&bytes, &mut term.core, &mut term.vbuf);
                }
                SshToGui::ConnectFailed(reason) => {
                    term.status = Some(format!("connect failed: {reason}"));
                }
                SshToGui::Closed => {
                    term.status = Some("session closed".to_string());
                }
                SshToGui::Disconnected(reason) => {
            ...
            ui.horizontal(|ui| {
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if ui.button("Disconnect").clicked() {
                        let _ = term.to_ssh.send(GuiToSsh::Close);
        ...
        egui::CentralPanel::default()
            ...
            .show(ctx, |ui| {
                ...
                if new_cols != term.cols || new_rows != term.rows {
                    ...
                    let _ = term
                        .to_ssh
                        .send(GuiToSsh::Resize(new_cols as u32, new_rows as u32));
                }
                ...
                let (rows, cursor) = compose_frame(&term.core, &mut term.vbuf, term.scroll_offset);
        ...
        if let Some(bytes) = collect_keyboard_input(ctx) {
            let _ = term.to_ssh.send(GuiToSsh::Input(bytes));
            term.scroll_offset = 0;
        }

        if back_to_launcher {
            let defaults = LauncherDefaults {
                host: String::new(),
                port: 22,
                username: String::new(),
                identity: None,
            ...
            self.screen = Screen::Launcher(Launcher::new(&defaults));
        }

        // Keep repainting while a session is live so incoming SSH data
...
/// send to the SSH channel, if any key that maps to something was
/// pressed. Regular printable text arrives via `egui::Event::Text`
/// (handles IME/shift/unicode for us); the explicit `Key` match below
/// only needs to cover control keys and Ctrl-letter combos, matching
